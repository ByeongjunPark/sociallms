...
[요약 미션 - 분리 요약 필수의무]
제공된 익명 의견들을 면밀히 취합하여, 반드시 다음 2가지 영역으로 나누어 마크다운 포맷으로 구조화해 요약해 주세요.

1. 🌸 **병준 선생님에 대한 첫인상 소감 요약** (학생들이 느끼는 선생님에 대한 첫인상이나 이미지)
2. 💡 **이번 학기 수업에서 선생님께 바라는 점 요약** (과제량 조절, 수업 속도, 수업 방식 등 건의 사항)

🚨 [경고 - 임의 창작(Hallucination) 절대 금지 및 팩트 기반 요약 원칙]
* 절대로 제공된 학생 의견 리스트에 존재하지 않는 가상의 건의사항이나 첫인상을 멋대로 지어내어(Hallucination) 요약하지 마십시오.
* 오직 제공된 의견 텍스트들에만 100% 근거하여 요약해야 합니다. 만약 의견들이 적다면 억지로 길게 포장하지 말고 실제 나타난 팩트만을 있는 그대로 짧고 정직하게 요약해 주세요.
    ...
    const response = await fetch("/api/chat", {
      ...
      body: JSON.stringify({
        messages: [
          {
            role: "system",
            content: "당신은 학교 현장에서 학생들의 익명 건의사항을 분석하는 신중하고 품위 있는 교무 장학 AI 비서입니다. 임의의 억측이나 없는 내용을 지어내는 할루시네이션을 전면 차단하고, 학생들의 사적인 민감 정보나 사생활 비밀은 요약문에서 절대로 노출시키지 않는 강력한 개인정보 보호 가드레일을 준수합니다."
          },
          {
            role: "user",
            content: prompt
