...
// 🔑 구글 시트 Users 탭 C열 비밀번호 직통 무조건 1:1 반환기
function getStudentPasswordFromRow(s) {
  if (!s || typeof s !== "object") return "비밀번호 미설정";

  // 1단계: 명시적 헤더 키 매칭 ("비밀번호", "password", "pw", "비번", "암호", "pin")
  for (const k of Object.keys(s)) {
    ...
    if ((cleanK.includes("비밀번호") || cleanK.includes("비번") || cleanK.includes("password") || cleanK.includes("pw") || cleanK.includes("pin") || cleanK.includes("암호")) && !cleanK.includes("캐릭터") && !cleanK.includes("emoji")) {
      ...
      if (val && val !== "undefined" && val !== "null") {
        return val; // C열 헤더 매칭 시 필터 없이 100% 무조건 직통 반환!
      }
    }
  }

  // 2단계: 순수 시트 3번째 컬럼 (Col C / index 2) 다이렉트 키 값 반환
  ...
  // 3단계: 순수 시트 3번째 값 (Col C / index 2 value) 다이렉트 반환
    ...
    return `
      <tr style="border-bottom: 1px solid rgba(0,0,0,0.04); transition: background 0.2s;">
        ...
        <td style="padding: 14px 8px; font-weight: 700; cursor: pointer;" onclick="showStudentPasswordModal('${sId}', '${safeName}', '${safePassword}')" title="클릭 시 4자리 이모티콘 비밀번호 확인 🔑">
          ...
          <span style="font-size: 0.76rem; color: #1d4ed8; font-weight: 800; display: inline-flex; align-items: center; gap: 3px; margin-left: 6px; background: rgba(59, 130, 246, 0.08); border: 1px solid rgba(59, 130, 246, 0.2); padding: 2px 8px; border-radius: 8px;">🔑 비번 확인</span>
        </td>
            ...
            ${(() => {
              const qQuestionsInfo = [
                ...
                { 
                  ...
                  question: "제130조 헌법개정(의무적/필수적 국민투표) vs 제72조 국가안위 중요정책(대통령 재량 선택적/임의적 국민투표)",
                  options: {
                    "정답": "정답 인정 (의무적/필수적 vs 선택적/임의적 핵심 차이점 서술)",
                    "오답": "보완 필요 (핵심 차이점 뉘앙스 누락)"
                  },
                  correct: "정답",
                  explanation: "헌법 개정은 의무적(필수적) 국민투표이며, 중요 정책은 대통령 판단에 따른 선택적(임의적) 국민투표임을 파악했습니다."
                }
              ];
